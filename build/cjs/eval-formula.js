...
function evalFormula(formula, context) {
    ...
    switch (formula === null || formula === void 0 ? void 0 : formula.type) {
        ...
        case 'constant': {
            var value_1 = formula.value;
            switch (formula.result_type) {
                case 'text':
                    return value_1;
                case 'number':
                    return parseFloat(value_1);
                default:
                    return value_1;
            }
        }
...
function evalFunctionFormula(formula, ctx) {
    ...
    switch (formula.name) {
        ...
        case 'min': {
            var values_1 = args.map(function (arg) { return evalFormula(arg, ctx); });
            return values_1.reduce(function (acc, value) { return Math.min(acc, value); }, Number.POSITIVE_INFINITY);
        }
        case 'mod':
            return (evalFormula(args[0], ctx) %
                evalFormula(args[1], ctx));
        ...
        // ------------------------------------------------------------------------
        case 'concat': {
            var values_2 = args.map(function (arg) { return evalFormula(arg, ctx); });
            return values_2.join('');
        }
        case 'contains':
            return evalFormula(args[0], ctx).includes(evalFormula(args[1], ctx));
        case 'format': {
            var value_2 = evalFormula(args[0], ctx);
            switch (typeof value_2) {
                case 'string':
                    return value_2;
                case 'object':
                    if (value_2 instanceof Date) {
                        return date_fns_1.format(value_2, 'MMM d, YYY');
                    }
                    else {
        ...
        case 'join': {
            ...
            var values_3 = restArgs.map(function (arg) { return evalFormula(arg, ctx); });
            return values_3.join(delimiter);
        }
        case 'length':
        ...
        case 'replaceAll': {
            var value_3 = evalFormula(args[0], ctx);
            var regex_1 = evalFormula(args[1], ctx);
            var replacement_1 = evalFormula(args[2], ctx);
            return value_3.replace(new RegExp(regex_1, 'g'), replacement_1);
        }
        case 'slice': {
            var value_4 = evalFormula(args[0], ctx);
            var beginIndex = evalFormula(args[1], ctx);
            var endIndex = args[2]
                ? evalFormula(args[2], ctx)
                : value_4.length;
            return value_4.slice(beginIndex, endIndex);
        }
        case 'test': {
            var value_5 = evalFormula(args[0], ctx);
            var regex_2 = evalFormula(args[1], ctx);
            return new RegExp(regex_2).test(value_5);
        }
        // date & time
        // ------------------------------------------------------------------------
        case 'date':
        ...
        case 'dateAdd': {
            var date_1 = evalFormula(args[0], ctx);
            var number_1 = evalFormula(args[1], ctx);
            var unit_1 = evalFormula(args[1], ctx);
            return date_fns_1.add(date_1, (_a = {}, _a[unit_1] = number_1, _a));
        }
        case 'dateBetween': {
            var date1 = evalFormula(args[0], ctx);
            var date2 = evalFormula(args[1], ctx);
            var unit_2 = evalFormula(args[1], ctx);
        ...
        case 'formatDate': {
            var date_2 = evalFormula(args[0], ctx);
            var formatValue = evalFormula(args[1], ctx).replace('dddd', 'eeee');
            return date_fns_1.format(date_2, formatValue);
        }
        case 'fromTimestamp':
